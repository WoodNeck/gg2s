/*
 *	GG2S: Gang Garrison 2 Stats with GAE support
 *	Author: WoodNeck
 *  http://www.ganggarrison.com/forums/index.php?topic=37158.0
 *
...
global.unique_key = '';
global.gg2stats_version = 'v1.5.1';
...
//Add ping checker
if !variable_global_exists('pingChecker')
{
	globalvar SimplePingChecker;
	SimplePingChecker = object_add();
	object_event_add(SimplePingChecker, ev_create, 0,"
		pingLimit = 300;
		pingRunning = true;
	...
	object_event_add(SimplePingChecker, ev_step, ev_step_begin, "
		if(pingRunning && pingLimit > 0) {
			if(!socket_connecting(pingSocket)) {
	...
	object_event_add(SimplePingChecker, ev_other, ev_user0, "
	...
	if(global.unique_key != '')
	{
		    ...
		    string_copy(global.currentMap, 0, 4) != 'gg2w')
		{
			...
			if (global.myself.stats[KILLS] > 0 or global.myself.stats[ASSISTS] > 0)
			{
				...
				//Find each player's most played team
				for(i = 0; i<ds_list_size(global.players); i += 1)
				{
					gg2s_player = ds_list_find_value(global.players, i);
					if (gg2s_player.team_timer[TEAM_RED] || gg2s_player.team_timer[TEAM_BLUE]) {
						if (gg2s_player.team_timer[TEAM_RED] >= gg2s_player.team_timer[TEAM_BLUE])
							gg2s_player.gg2s_latest_team = TEAM_RED;
						else
							gg2s_player.gg2s_latest_team = TEAM_BLUE;
					}
					else
						gg2s_player.gg2s_latest_team = TEAM_SPECTATOR;
				}
				...
				if (global.isHost)
					write_string(httpHandler.http_buffer, '&match_ishost=' + '1');
				else
					write_string(httpHandler.http_buffer, '&match_ishost=' + '0');
				
				gg2s_stat_update = instance_create(0, 0, SimplePingChecker);
